...
- **Migration `035_memory_session_record.sql`** (T2, non-superuser-applied on pg17, hard-failing receipt):
  - **`memory_sessions` provenance columns** — `metadata jsonb not null default '{}'` (reconciles mig-001's canonical column onto the live rag-system-shaped table, the 017-direction; code written against canonical `.metadata` no longer 42703s on rag-system-shaped stores) + `source_agent text` (NULL = CLI/hook-written; non-NULL = `*-web` — the first way to distinguish a web-recorded session row from a panel-close row). Both `add column if not exists`, no rewrite of existing rows.
  - **`memory_session_record(...)` RPC** (SECURITY DEFINER, pinned `search_path`, EXECUTE revoked from PUBLIC/anon/authenticated, service_role only — mig-026's five-gate shape). **`session_id` is minted server-side, never caller-supplied** (`'web:' || source_agent || ':' || conversation_key`) — the load-bearing defense: a web caller structurally cannot address, and cannot overwrite, a CLI/hook-written session row (whose `session_id` is a bare session UUID). The upsert is narrowed twice more: `do update` fires only where the existing row's `source_agent` matches AND `rumen_processed_at is null` — a caller can amend its own not-yet-swept record but can never re-arm an already-consumed row for a second Rumen synthesis pass. 15 rejection reason codes under the stable `MEMORY_SESSION_RECORD_REJECTED:` prefix (mirrors 026's convention; incl. `invalid_conversation_key` charset-bounding the only caller-controlled `session_id` component, `ended_at_in_future` +24h-bounded so a future timestamp can't pin the row inside the Rumen picker's lookback window forever, `session_locked`).
  - **Closes a live gate-5 drift**: on a clean 001→034 chain `memory_sessions` ends with RLS **OFF** (the daily driver's ON state was hand-applied, not chain-applied — a fresh install inherited the gap). 035 enables it idempotently, revokes the Supabase auto-grant residue (anon/authenticated held full table grants), and the receipt hard-fails if any survive.
- **Migration `036_memory_inbox_hygiene.sql`** (T3, same non-superuser bar): **`purge_memory_inbox(p_retention_days int default 90)`** — deletes SETTLED (`status <> 'pending'`) inbox rows past retention, modelled line-for-line on 027's `purge_recall_log`; pending rows are untouchable at every retention value (the promoter's claim lease lives in `metadata.rumen` and stays `pending`, so a claim can never make a row purgeable). **`memory_inbox_health` view** (`security_invoker = true` — inherits `memory_inbox`'s RLS instead of leaking as a definer view): one row of `pending_count` / `oldest_pending_age_hours` / 24h throughput / `alarm` (>7d pending = the promotion drain is dead) — evaluated at read time by whoever asks, so it is still correct when every promoter run has failed (the alarm must not be computed by the drain it watches). `mnestra-inbox-purge` pg_cron @ 04:20 UTC, registered fail-soft only when pg_cron exists.
- **`src/session_record.ts`** TS mirror + `SessionRecordInput`/`SessionRecordResult` in `src/types.ts`; **`session_record` webhook op** in `src/webhook-server.ts` — returns **501 when a pre-84 deps object is in play, never a 200 for a write that did not happen** (the Sprint-66 silent-no-op lesson applied to the write path).
- Tests: `tests/session-record.test.ts`, `tests/webhook-session-record.test.ts`, `tests/migration-035-hygiene.test.ts`.

### Notes
- Sprint 84 ("Write-Side Completion") T2 + T3 engram half, **FINAL-VERDICT GREEN** (T4 Codex adversarial auditor; the sprint's 6 audit catches were all fixed in-flight, incl. the 035/036 migration-number collision resolved by ORCH ruling R4 before either lane built against a filename). `npm test` **458 (456 pass / 2 pre-existing skips)**, typecheck clean, gitleaks 0. T2's behaviour matrix + T4's independent non-superuser replay both green (hostile web-caller-passes-CLI-UUID case, cross-agent isolation, `session_locked`, 13/15-code rejection matrix, Rumen picker WHERE clause pasted verbatim selects the web rows).
- **Migrations 035/036 NOT applied to the daily driver at publish** — ORCH close-out step (order: 035 → 036; receipts hard-fail).
- BACKLOG'd in termdeck: the RLS-was-off-on-clean-chains pattern — audit whether OTHER tables share the hand-enabled-not-in-chain drift.
- Companions: `@jhizzard/rumen@0.11.0` (extraction sweep) + `@jhizzard/termdeck@1.17.0` + `@jhizzard/termdeck-stack@1.15.0`.

## [0.11.0] - 2026-07-31

### Added — graph layer: edge temporality + FK-governed vocabulary + entities + the recall-citation label producer (Sprint 83 T1/T2)
- **Migration `034_graph_layer.sql`** (2,137 lines, apply-receipt hard-failing, idempotent on re-apply):
  - **Edge temporality on `memory_relationships`**: `valid_at`/`invalid_at`, backfilled add-nullable → `set valid_at = created_at` → default → not-null (a volatile-default add would have stamped all 7,4xx live edges with apply-time instead of their true creation time). **Invalidate, never DELETE** — statically fenced (the hygiene suite asserts no `DELETE` anywhere in 034). Partial live-traversal indexes on `(source_id, relationship_type)` / `(target_id, relationship_type)` `WHERE invalid_at IS NULL`.
  - **14-predicate vocabulary as a lookup table + FK** (`memory_relationship_types`), replacing the CHECK idiom (001/009/028 each re-authored the full list and 009's is stale on disk): existing 10 preserved + `same_pattern_as` `fixed_by` `documented_at` `part_of`, with per-predicate direction semantics documented in-migration. **Backward compat is mechanical, not inventory-based**: before the FK lands, 034 adopts `SELECT DISTINCT relationship_type` from live edges, so an install carrying a predicate this file never heard of still applies — CI's pre-034 legacy fixture (`tests/sql/034a`) makes deleting that pass a migration failure, not a silent install-breaker.
  - **Entity storage**: `memory_entities` (dedup key `(entity_type, entity_key)`, `entity_key = lower(btrim(name))` server-side) + `memory_entity_mentions` join table + `memory_entity_types` (12 seed types). All RLS-enabled, zero policies, service-role-only.
  - **`mark_recall_cited_group(p_recall_group_id, p_ranks, p_memory_ids, p_source_agent)`** — the label producer's SQL surface: **group-keyed**, not most-recent-per-memory (concurrent panels can no longer mis-attribute a citation to another panel's reinjection event); idempotent in state AND return (post-condition count; honest 0 on unknown/stale group); `source_agent` NULL-only fill; **`group_resolved_at` stamped on every row of the group on first cite** — the uncited complement becomes an *observed* negative, kept strictly separate from `dismissed`.
  - **`memory_expand_typed(p_seed_ids, p_predicates, p_max_depth, p_max_rows, p_project)`** — typed/temporal 1–2 hop bidirectional expansion, `STABLE` (structural read-only proof — receipt hard-fails if it ever goes VOLATILE), INVOKER, single overload: live-edges-only, typed-only paths (default allowlist `{caused_by, fixed_by, supersedes, same_pattern_as}`), tombstone hygiene on every path node, `privacy_tags` passthrough, caps clamped inside the function, dedup + no seed echo, `direction` of the last edge + full `edge_path` + mean `edge_weight`. `expand_memory_neighborhood` replaced at the **identical signature** (no second overload — PostgREST resolves by JSON key set) to add `invalid_at IS NULL`: bit-identical today, correct from the first invalidation forward.
  - **`upsert_memory_edges(p_edges)` / `upsert_memory_entities(p_memory_id, p_entities)`** — batch write RPCs, drop-invalid per element with **enumerated data-error condition lists and no bare `when others`** (the audit lesson: a catch-all handler turned a function defect — `xmax = 0::xid`, an impossible cast — into clean "dropped" counts; a broken function is now loud, a broken edge is dropped). Resurrection semantics folded in: `ON CONFLICT` clears `invalid_at` so a re-asserted edge comes back live instead of staying invisible to every live-only traversal.
  - **§2c repair of 033's `solved-problem` decay profile**: the `debugging`/`convention` CASE arms keyed on `source_type`, which 028's CHECK makes unreachable — the profile covered 252 of ~631 solved-problem-class rows (40%). The debugging arm now keys on `category = 'debugging'`, the dead `convention` arm is dropped; coverage 252 → 576, and every solved-problem-class row that *could* be buried is now protected. `memory_hybrid_search` replaced at the identical 10-arg signature with both `SET` clauses restated in-statement (the proconfig trap: `CREATE OR REPLACE` without them nulls the `search_path` pin AND 033's `hnsw.ef_search=120`); the explain sibling inherits by delegation.
  - `problem_signature` expression indexes on `metadata->'problem_signature'->>'class'` / `->>'symptom_hash'` (`WHERE metadata ? 'problem_signature'`); partial unique consolidation community-key index; `source_type` CHECK widened with `'consolidation_summary'` (receipt asserts both directions — value added AND none of the 11 pre-existing values dropped).
- **Label producer, client side**: `recall_group_id` minted in `memoryRecall`/`memoryIndex`/`memoryTimeline` and returned end-to-end (`RecallOutput.recall_group_id` required, per-hit, webhook body) — previously it was minted *inside* the fire-and-forget logger and never escaped, so no agent could name what it just recalled. Recall text lines carry **`[n]` citation handles** (~4 chars vs a 36-char uuid on the one token-budgeted surface) + a trailing cite-only-what-you-used prompt. New **`memory_cite` MCP tool** (`src/cite.ts`): rank- and/or id-narrowed, one round-trip, diagnostic lookup only on an honest zero. **Acceptance proven the consumer's way**: the DB-backed round-trip asserts `fit-platt.ts`'s own label query goes `positives` 0 → 1 (rank-targeted), plus a **negative control** proving cited `surface='graph'`/NULL-score rows stay outside the fit window.
- **`problem_signature` computed inline on solved-problem-class writes** (`source_type==='bug_fix' || category==='debugging'`; pure regex, no network — needs no budget guard): object under ONE metadata key (atomic under dedup shallow-merge), keep-canonical on reinforcement unless `refresh`. Normalizer core is a **dependency-free dual-export `src/problem_signature_core.cjs`** (CJS `require` + ESM import; secret-shape redaction inlined and parity-tested) with 12 golden vectors both repos run; `scripts/copy-js-assets.mjs` wired into **build and test** (`tsc` never emits a hand-written `.cjs` — without the copy step `dist/` fails at runtime on the first bug_fix write, not at build time).
- **Write-time entity/triple extraction** (`src/extract_write.ts`): fail-open proven by 11 cases (`memoryRemember` still returns `'inserted'` when every extraction surface is broken), AbortSignal budget guard, vocabulary read live from the FK tables (never transcribed into TS), 404-capability-latched against pre-034 stores, deterministic `same_pattern_as` edges land even with no API key. **OFF by default** (`MNESTRA_EXTRACT_ENABLED=1`) — a model call per write must be opted into, not inherited at upgrade.

### Notes
- Sprint 83 ("Graph Layer + Label Producer") T1 + T2, **FINAL-VERDICT GREEN + GREEN-REAFFIRMED** (T4 Codex adversarial auditor; zero RED verdict cycles — the defects were caught in-flight pre-landing: the `xmax` cast + catch-all-handler narrowing, the dead top-level signature index, the pre-ruling contract drift). `npm test` **407 (405 pass / 2 DB-skip)**, typecheck clean, gitleaks 0. CI now runs the apply-split 001→032 → 033a → 033 → **034a (pre-034 legacy fixtures) → 034 → 034b** behavioral acceptance against real pgvector.
- **Migration 034 NOT yet applied to the daily driver at publish** — ORCH close-out step (§11 of the migration is the checklist, incl. the `get_advisors` five-gate sweep).
- Known gaps, deliberately not papered over (BACKLOG'd in termdeck): `ingest_capture` writes bypass extraction (SQL-direct over PostgREST, no TS in the path — needs a rumen sweep phase); entity-level triples are extracted but unpersisted (no entity↔entity edge table — SR-7).
- Companions: `@jhizzard/rumen@0.10.0` + `@jhizzard/termdeck@1.16.0` + `@jhizzard/termdeck-stack@1.14.0`.

## [0.10.0] - 2026-07-30

### Added — two-phase index-served hybrid search + honest calibration contract (Sprint 82 T1/T3)
- **Migration `033_two_phase_hybrid_search.sql`**: two-phase top-k rewrite of `memory_hybrid_search` — each branch takes its own **index-served** top-k first (both branch CTEs `MATERIALIZED` so PG12+ inlining can't silently undo the `LIMIT`), then RRF fusion + the whole multiplier stack run only on the union (≤2×`p_branch_limit`, ≤120 rows at defaults) instead of 032's whole-corpus scan. Creates **`memory_items_content_fts_gin`** on `gin(to_tsvector('english', content))` — the first FTS GIN index the store has ever had (001's index is `gin_trgm_ops`, which cannot serve `@@`; the FTS branch has been seq-scanning through the whole 002→032 lineage — the documented cause of the 57014 recall timeouts per 024's own header comment). Creates **partial `memory_items_embedding_hnsw_live_idx`** (`WHERE is_active AND NOT archived AND superseded_by IS NULL` — exactly the live-row predicates every vector query applies) so the vector branch is HNSW-index-servable; `hnsw.ef_search` pinned at 120 at the function level (pgvector default 40 < the default branch limit 60).
- **`semantic_similarity double precision`** appended to RETURNS TABLE (col 10): raw cosine `1 - (embedding <=> query_embedding)`, recomputed on the fused set so FTS-only rows carry it too; NULL only when there is no query embedding. The **cardinal**, cross-query-comparable signal — the RRF composite `score` is ordinal with a hard ~0.0738 ceiling and stays untouched.
- **`p_decay_profile text default 'standard'`** — `'solved-problem'` flattens `bug_fix`/`debugging` recency half-life 30d→365d, every other type byte-identical; unknown/NULL silently falls back to `'standard'`. (The `'debugging'` arm is unreachable today — Category-vs-SourceType vocabulary drift, see Notes.) **`p_branch_limit int default 60`** (clamped `[match_count, 500]`). Deterministic tie-breaks in both rank windows + the final ORDER BY (032 had none — top-k boundaries could shuffle between identical calls). A NULL query embedding no longer fabricates semantic ranks from `nulls last` noise — the branch gates off and degrades honestly to full-text-only.
- **`memory_hybrid_search_explain` kept in 10-arg lockstep with an explicit grant re-pin** — adding defaulted params requires DROP+recreate, and a DROP hands PUBLIC back default EXECUTE on an `EXPLAIN (ANALYZE)` wrapper that runs the query for real; 033 re-pins and its receipt asserts exactly one overload of each function. Hard-failing OID-form receipt covers RLS, both indexes, function shape, privileges, pinned `search_path`, no SECURITY DEFINER (all five hygiene gates).
- **`src/calibration.ts` — label-gated calibration contract** (Sprint 82 T3): `CALIBRATION_VERSION`, `CALIBRATION_FITTED = false`, `calibrateScore() → null` until a real fit exists — `score_calibrated` is **absent** (not null, not 0) from recall output, so `'score_calibrated' in hit` is a truthful "has anyone estimated this" test, and `withCalibratedScore` returns the input array by identity on the unfitted path. `scoreBandPercentile()` — the label-free empirical-CDF percentile, same 14-knot quantile table as rumen 0.9.0's confidence v3, kept byte-identical on purpose (the two packages must not disagree about what an RRF score means).
- **`scripts/calibration/fit-platt.ts`** — read-only Platt-fit harness (every statement a SELECT **and** the session pinned `transaction read only`; DSNs scrubbed from every error path) with an EPV-gated refuse-to-fit: below `max(100 absolute, 13 features × 20 EPV)` positives it emits an `INSUFFICIENT LABELS` report instead of fabricated coefficients. Ran live: 2 positives (both test artifacts) / 0 dismissals over ~39k telemetry rows → refused. **`docs/calibration-report-2026-07-30.md`** is script-generated, with a self-measuring quantile-snapshot drift section (pinned snapshot 2026-07-30 20:11 ET, n = 39,048, live re-measurement with per-knot deltas on every rerun).

### Changed
- TS pass-through of `semantic_similarity` in `recall.ts` / `search.ts` / `layered.ts` + webhook surface; `RecallHit` gains `semantic_similarity` and additive `score_calibrated?`. All three RPC callers use named args — zero call-site edits needed for the two new params.
- `ci.yml` now runs the DB-backed 033 acceptance against real `pgvector/pgvector:pg16`: applies 001→032, captures the **real** 032 function's output as baseline, applies 033, runs `tests/sql/033b_verify.sql` — equivalence via exact rank-order identity (zero tolerance) + a clock-drift-derived score tolerance, cosine exactness, solved-problem reorder + nothing-else-moves, NULL-profile/NULL-embedding degradation, §7 index-plan assertions (FTS GIN **choice** asserted; HNSW **servability** asserted under `enable_sort=off`, planner choice recorded not asserted — a 1.2k-row fixture cannot honestly promise a cost decision), five hygiene gates re-checked independently.
- `package.json`: `pg` + `@types/pg` added as **devDependencies** only (`scripts/` sits outside both the tsconfig include and the npm `files` whitelist — nothing new ships in the tarball).

### Notes
- Sprint 82 ("Calibration & Solved-Problem Recall") T1 + T3 engram half, **FINAL-VERDICT-4 GREEN** (T4 Codex adversarial auditor; 3 RED cycles → GREEN, ~60 min inject→verdict). `npm test` **332/332** (16-test migration-033 hygiene suite grown with mutation-verified guards: parsed-028-allow-list fixture guard, derived-tolerance guards, HNSW-predicate-parity guard; 17 calibration tests; DB-backed `tests/sql/033a`+`033b` in CI), `typecheck` + `typecheck:scripts` clean, gitleaks 0.
- **Migration 033 APPLIED LIVE to the daily driver at close-out** (ORCH): receipt green, both EXPLAIN plans index-served at ~9.3k rows (vector branch 3.1 ms). 73 polluting smoke-test telemetry rows purged from `memory_recall_log` in the same pass.
- Known vocabulary drift, ruled leave-documented: `debugging`/`convention` exist as `Category` but are not legal `source_type` under 028's CHECK, so their decay arms (inherited since 002) are unreachable — BACKLOG'd for deliberate reconciliation, not silently widened.
- Headline sprint finding (T3-F1): **the recall-telemetry label channel has no producer on the dominant path** — ordinary `memory_recall` returns content inline, nothing emits citations, so ~39k logged rows carry 0 real positives; every "data-driven" threshold (calibration, pruning, elevation) is blocked on a label producer. Sprint-83-shaped. Companions: `@jhizzard/rumen@0.9.0` + `@jhizzard/termdeck@1.15.0` + `@jhizzard/termdeck-stack@1.13.0`.
